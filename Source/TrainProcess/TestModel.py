...
class TestModel(object):
                ...
                for i in xrange(self.args.gpu):
                    with tf.device('/gpu:%d' % i):
                        ...
                        with tf.name_scope('%s_%d' % ('TOWER', i)):
                            ...
                            # get the result
                            _, _, coarse_map, refine_map = MSGFNet().NetWork(
    ...
    def TestProcess(self, args, randomTestList, num_test_batch):
        ...
        for step in range(num_test_batch):
            imgLs, imgRs, names = GetBatchDFCTestImage(
                args, randomTestList, data_imgLs, step, False)
            res = self.sess.run(
            ...
            for i in range(args.gpu):
                for j in range(args.batchSize):
                    ...
                    SaveDFCTestImg(args, temRes, names[i*args.batchSize + j])
