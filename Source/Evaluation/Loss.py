...
def MAE_Loss(result, labels):
    ...
    # check the right map
    mask1 = tf.cast(labels <= IMG_DISPARITY / 2, dtype=tf.bool)
    loss_ = tf.where(mask1, loss_, tf.zeros_like(loss_))
    mask2 = tf.cast(labels >= -IMG_DISPARITY / 2, dtype=tf.bool)
...
def L2_loss(loss, alphi=1):
    ...
    # get the loss
    loss_final = tf.add_n([loss] + alphi * regularization_losses)
