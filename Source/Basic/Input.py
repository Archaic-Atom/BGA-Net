...
def RandomCropRawImage(args, num):
    ...
    imgL = ReadImg(pathL)
    x, y = RandomOrg(imgL.shape[2], imgL.shape[1], w, h)
...
class Dataloader(object):
    ...
    def __init__(self, args):
        ...
        self.trainQueue = Queue.Queue(maxsize=10)
        self.valQueue = Queue.Queue(maxsize=10)
        self.exitFlag = 0
...
def GetBatchDFCTestImage(args, randomlist, data_imgLs, num, isVal=False):
    ...
    for i in xrange(args.batchSize * args.gpu):
        idNum = randomlist[args.batchSize * args.gpu * num + i]
        imgL, imgR, name = GetDFCTestImg(args, data_imgLs, idNum)       # get img
