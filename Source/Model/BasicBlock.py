...
def ExtractUnaryFeatureBlock5(x, training=True):
    with tf.variable_scope("ExtractUnaryFeatureBlock5"):
        x = Conv2DLayer(x, 3, 1, 512, "Conv_1", training=training)
        x = Conv2DLayer(x, 3, 1, 256, "Conv_2", biased=True,
...
def MatchingBlock(x, training=True):
    with tf.variable_scope("MatchingBlock"):
        ...
        x = Conv3DLayer(x, 3, 1, 32, "Conv_1", training=training)
        x = Res3DBlock(x, 3, 1, "Res_1", training=training)
    return x
...
def EncoderBlock(x, training=True):
    with tf.variable_scope("EncoderBlock"):
        level_list = []
        # 1/8
        x = DownSamplingBlock(x, 3, 64, "Level_1", training=training)
        level_list.append(x)

        # 1/16
        x = DownSamplingBlock(x, 3, 96, "Level_2", training=training)
        level_list.append(x)

        # 1/32
        x = DownSamplingBlock(x, 3, 128, "Level_3", training=training)
        level_list.append(x)

        # 1/64
        x = DownSamplingBlock(x, 3, 160, "Level_4", training=training)
    return x, level_list
...
def DecoderBlock(x, level_list, training=True):
    with tf.variable_scope("DecoderBlock"):
        # 1/32
        x = UpSamplingBlock(x, 3, 128, "Level_3", training=training)
        x = x + level_list[2]

        # 1/16
        x = UpSamplingBlock(x, 3, 96, "Level_2", training=training)
        x = x + level_list[1]

        # 1/8
        x = UpSamplingBlock(x, 3, 64, "Level_1", training=training)
        x = x + level_list[0]
...
def RecoverSizeBlock(x, training=True):
    with tf.variable_scope("RecoverSizeBlock"):
        # 1/2
        x = DeConv3DLayer(x, 3, 2, 16, "DeConv_1", training=training)
...
def GetWeightBlock(batchsize, disp_num, height, width):
    disp = tf.range(-disp_num / 2, disp_num / 2, 1.0)
...
def FeatureConcat(x, imgL, seg, training=True):
    with tf.variable_scope("FeatureConcat"):
        ...
        seg = Conv2DLayer(x, 1, 1, 32, "Conv_1", training=training)
...
def RecoveryCLSSizeBlock(x, cls_num, training=True):
    with tf.variable_scope("RecoveryCLSSizeBlock"):
        ...
        x = DeConv2DLayer(x, 3, 2, 16, "DeConv_2", biased=True,
                          relu=False, bn=False, training=training)
